...
void Show2dArraySimpl(double[,] array)
{
    for (int i = 0; i < array.GetLength(0); i++)
    {
        for (int j = 0; j < array.GetLength(1); j++)
...
void ShowElement(int[,] array, int a, int b)
{
    ...
    for (int i = 0; i < array.GetLength(0); i++)
    {
        ...
        {
           if(i ==a && j==b) 
           Console.Write($" Element with row {a} and colum {b} is {array[i,j]}");
        }  
    }
    }
    else Console.WriteLine("This element in this massiv isn't .");
}
...
Console.WriteLine("Input a element of row");
int m = Convert.ToInt32(Console.ReadLine());

Console.WriteLine("Input a element of columns");
int n = Convert.ToInt32(Console.ReadLine());
...
double[] arr = ArithmeticMeanColumOfMatrix(myArray);
ShowArray(arr);